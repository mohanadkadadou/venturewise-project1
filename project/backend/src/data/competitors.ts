...
export function getCompetitorsForBusiness(businessType: string, categoryName: string): Competitor[] {
  // Direct match
  if (COMPETITOR_MAP[businessType]) {
    return COMPETITOR_MAP[businessType];
  }

  // Fuzzy match by business type keywords
  const btLower = businessType.toLowerCase();
  for (const [key, comps] of Object.entries(COMPETITOR_MAP)) {
    const keyLower = key.toLowerCase();
    if (btLower.includes(keyLower.split(" ")[0]) || keyLower.includes(btLower.split(" ")[0])) {
      return comps;
    }
  }
  ...
  if (catLower.includes("food") || catLower.includes("restaurant") || catLower.includes("beverage")) {
  ...
  if (catLower.includes("health") || catLower.includes("medical")) {
    return COMPETITOR_MAP["Healthcare Clinic"];
  }
  if (catLower.includes("fashion") || catLower.includes("retail") || catLower.includes("apparel")) {
    return COMPETITOR_MAP["Retail Store"];
  }
  if (catLower.includes("consult")) {
    return COMPETITOR_MAP["Consulting Firm"];
  }
  if (catLower.includes("education") || catLower.includes("training")) {
    return COMPETITOR_MAP["Education & Training Center"];
  }
